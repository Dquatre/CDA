...
namespace CrudWithBdd.Controller
{
    public class BatimentsController : ControllerBase
    {
        ...
        public ActionResult<Batiment> CreateBatiment(BatimentsDtoOut objDto)
        {
            Batiment obj = new Batiment();
        ...
        public ActionResult PartialBatimentUpdate(int id, JsonPatchDocument<Batiment> patchDoc)
        {
            try
            {
                var objFromRepo = _service.GetBatimentById(id);

                var objToPatch = _mapper.Map<Batiment>(objFromRepo);
